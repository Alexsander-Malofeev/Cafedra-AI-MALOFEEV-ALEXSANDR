...
def compute_quality_flags(df: pd.DataFrame) -> dict:
    ...
    # НОВАЯ ЭВРИСТИКА №2 — слишком много уникальных значений в категориях
    ...
    score = 100
    if flags["has_missing_values"]: score -= 30
    if flags["has_duplicates"]: score -= 20
    if flags["has_constant_columns"]: score -= 15
    if flags["has_high_cardinality_categoricals"]: score -= 10
