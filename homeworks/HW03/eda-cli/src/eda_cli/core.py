...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    #Проверка на большую кардинальность категориальных признаков
    ...
    if summary.n_cols > 100:
        score -= 0.1
    if has_constant:  # константные колонки это плохо
        score -= 0.1
    if high_cardinality:  # большая кардинальность это плохо
        score -= 0.1
